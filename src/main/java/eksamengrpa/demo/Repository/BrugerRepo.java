...
public class BrugerRepo {
    ...
    public Bruger findByEmail(LoginAuthenticator l){
        ...
        return template.queryForObject(sql, rowMapper, l.getEmail());
    }

    public Bruger findById(int id){
        String sql = "SELECT bruger_id, bruger_firstName AS firstName, bruger_lastName AS lastName, bruger_email AS email, bruger_telephone AS telephone, bruger_gender AS gender, bruger_age AS age, bruger_country AS country FROM bruger WHERE bruger_id = ?";
        RowMapper<Bruger> rowMapper = new BeanPropertyRowMapper<>(Bruger.class);
